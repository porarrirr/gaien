...
class HomeViewModelTest {
    ...
    fun `state has goal when weeklyGoal is set`() = runTest {
        ...
        val homeData = createDefaultHomeData().copy(weeklyGoal = goal)
    ...
    fun `state has empty exams list when no exams`() = runTest {
        ...
        val state = viewModel.uiState.value
        assertTrue(state.upcomingExams.isEmpty())
    }
    ...
    fun `state has exams when exams are returned`() = runTest {
        ...
        val homeData = createDefaultHomeData().copy(upcomingExams = exams)
        every { getHomeDataUseCase() } returns flowOf(homeData)
    ...
    fun `todaySessions preserves order from UseCase`() = runTest {
        ...
        val sessions = listOf(
            TodaySession(id = 2L, subjectName = "English", materialName = "", duration = 2000L, startTime = now),
            TodaySession(id = 1L, subjectName = "Math", materialName = "", duration = 1000L, startTime = now - 1000),
            TodaySession(id = 3L, subjectName = "Science", materialName = "", duration = 3000L, startTime = now - 2000)
        )
        val homeData = createDefaultHomeData().copy(todaySessions = sessions)
        every { getHomeDataUseCase() } returns flowOf(homeData)
